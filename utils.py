...
class SeamImage:
    ...
    def calc_gradient_magnitude(self):
        ...
        # normalize magnitude to be in range [0, 1]
        gradient_arr = gradient_arr / np.max(gradient_arr)
...
class VerticalSeamImage(SeamImage):
    ...
    def calc_M(self):
        ...
        """
        # Initialize M with the same size as the energy matrix E
        m_img = np.zeros_like(self.E)

        # The first row of M is the same as the first row of E
        m_img[0, :] = self.E[0, :]
